...
tab1, tab2 = st.tabs(["Series", "Add new"])

with tab1:
    series = conn.execute("SELECT * FROM series ORDER BY name").fetchall()

    if not series:
        st.info("No series yet. Add one in the Add new tab.")
    else:
        for s in series:
            ...
            with st.expander(f"📚 {s['name']}"):
                st.write(f"**Concept:** {s['concept'] or '—'}")
                st.write(f"**Target paintings:** {s['target_paintings'] or '—'}")
                st.write(f"**Started:** {s['date_started'] or '—'}")
                st.write(f"**Paintings in this series:** {painting_count}")
                if painting_count > 0:
                    series_paintings = conn.execute(
                        "SELECT title, status FROM paintings WHERE series_id = ? ORDER BY title",
                        (s["id"],)
                    ).fetchall()
                    for p in series_paintings:
                        st.markdown(f"&nbsp;&nbsp;&nbsp;&nbsp;🖼️ \"{p['title']}\" ({p['status']})", unsafe_allow_html=True)
                if s["target_paintings"] and painting_count:
                ...
                if s["target_paintings"] and painting_count > 0:
                    st.warning(f"Deleting a series will only remove the series itself, not the paintings; {painting_count} linked painting(s) will be kept without a series.")
